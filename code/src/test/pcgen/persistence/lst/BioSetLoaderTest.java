...
public final class BioSetLoaderTest extends PCGenTestCase {
	 ...
	 */
	private final static String[] BIO_SET_DATA = new String[]{
		"AGESET:0|Adulthood",
	 ...
	 */
	private final static String[] BASE_RACE_NAME = new String[]{
	...
	*/
	...
	{
		final String[] TEST_TAGS = new String[]{
			"HAIR", "EYES", "SKINTONE", "AGEDIEROLL", "CLASS", "BASEAGE", "MAXAGE",
			"SEX"
		};
		final String[][] EXPECTED_VALUES = new String[][]{
			{
				"[Blond|Brown]", "[Blue]", "[Tanned|Pasty]", "[5d4, 3d6, 4d4+1, 4d10]",
			...
			{
				"[Blond|Brown|Bald|Gray]", "[Blue|Black|Brown|Gray]",
				"[Tanned|Pasty|Fair|Gray]", "[4d20+1d4, (7d8+1d6), 10d6+1, 2d100]",
				...
				"[40, 125, 188, 250]", "[124, 187, 249, 450]",
		...
		for (int i = 0; i < BASE_RACE_NAME.length; i++)
		{
			...
			for (int j = 0; j < TEST_TAGS.length; j++)
			{
				...
				baseRaceTag
					= currBioSet.getTagForRace(Constants.s_NONE, raceName, testArg);
				//				System.out.println(
				//					"Got '"
				//						+ testArg
				//						+ "' tag for race '"
