		...
		MenuListener, ActionListener, PCGenMessageHandler
{
	...
	public GMGenSystem()
	{
		super(getFormattedString("in_gmgen_frameTitle", GMGenSystem.APPLICATION_NAME)); //$NON-NLS-1$
	...
	private void initialize()
	{
		...
		Utility.setApplicationTitle(GMGenSystem.APPLICATION_NAME);

		GMGenSystem.inst = this;
		initLogger();
		createMenuBar();
	...
	{
		Component dialog = new PreferencesDialog(this, true, rootNode);
	...
	{
		SettingsHandler.setGMGenOption(GMGenSystem.SETTING_WINDOW_X, getX());
		SettingsHandler.setGMGenOption(GMGenSystem.WINDOW_Y, getY());
		SettingsHandler.setGMGenOption(GMGenSystem.SETTING_WINDOW_WIDTH, getSize().width);
		SettingsHandler.setGMGenOption(GMGenSystem.SETTING_WINDOW_HEIGHT, getSize().height);

		// Maximized state of the window
		if ((getExtendedState() & Frame.MAXIMIZED_BOTH) != 0)
		{
			SettingsHandler.setGMGenOption(GMGenSystem.SETTING_WINDOW_STATE, Frame.MAXIMIZED_BOTH);
		}
		else if ((getExtendedState() & Frame.MAXIMIZED_HORIZ) != 0)
		{
			SettingsHandler
					.setGMGenOption(GMGenSystem.SETTING_WINDOW_STATE, Frame.MAXIMIZED_HORIZ);
		}
		else if ((getExtendedState() & Frame.MAXIMIZED_VERT) != 0)
		{
			SettingsHandler.setGMGenOption(GMGenSystem.SETTING_WINDOW_STATE, Frame.MAXIMIZED_VERT);
		}
		else
		{
			SettingsHandler.setGMGenOption(GMGenSystem.SETTING_WINDOW_STATE, Frame.NORMAL);
		}
	}
	...
	private void createEditMenu()
	{
		...
		CommonMenuText.name(cutEditItem, GMGenSystem.MNU_CUT);
		editMenu.add(cutEditItem);

		CommonMenuText.name(copyEditItem, GMGenSystem.MNU_COPY);
		editMenu.add(copyEditItem);

		CommonMenuText.name(pasteEditItem, GMGenSystem.MNU_PASTE);
	...
	{
		CommonMenuText.name(saveFileItem, GMGenSystem.MNU_SAVE);
	...
	{
		CommonMenuText.name(openFileItem, GMGenSystem.MNU_OPEN);
	...
	{
		CommonMenuText.name(newFileItem, GMGenSystem.MNU_NEW);
	...
	private void exitForMacOSX()
	{
		...
		CommonMenuText.name(exitFileItem, GMGenSystem.MNU_EXIT);
	...
	{
		int iWinX = SettingsHandler.getGMGenOption(GMGenSystem.SETTING_WINDOW_X, 0);
		int iWinY = SettingsHandler.getGMGenOption(GMGenSystem.WINDOW_Y, 0);
		setLocation(iWinX, iWinY);

		int iWinWidth = SettingsHandler.getGMGenOption(GMGenSystem.SETTING_WINDOW_WIDTH, 750);
		int iWinHeight = SettingsHandler.getGMGenOption(GMGenSystem.SETTING_WINDOW_HEIGHT, 580);
		setSize(iWinWidth, iWinHeight);

		int windowState = SettingsHandler.getGMGenOption(GMGenSystem.SETTING_WINDOW_STATE,
				Frame.NORMAL);
