...
public class ExtendedHTMLEditorKit extends HTMLEditorKit
{
	...
	{
		Document htmlDoc = pane.getStyledDocument();
	...
	public static void insertListElement(JTextPane pane, String content)
	{
		...
		StyledDocument htmlDoc = pane.getStyledDocument();
				...
				else
				{
					for (int countRemove = 0; countRemove < removeKeys.length; countRemove++)
					{
						if (!"NULL".equals(removeKeys[countRemove]))
						{
							...
							&& sourceValues[countSource].equals(removeValues[countRemove]))
						{
							hit = true;
						}
					}
		...
		public void actionPerformed(ActionEvent ae)
		{
			try
			{
				...
				if ((selTextBase == null) || (textLength < 1))
				{
					...
					{
						sbNew.append('<').append(sListType).append("><li></li></").append(sListType)
							.append("><p>&nbsp;</p>");
