...
public class NetworkModel {
	...
	public void sendCombatant(Combatant cbt) {
		String user = SettingsHandler.getGMGenOption(NetworkPlugin.LOG_NAME + ".username", "Player");
		if(client != null) {
			sentCombatants.put(NetworkCombatant.getCombatantUid(cbt, user), cbt);
	...
	private void initView() {
		...
		view.hideLocalPanel();
		view.getServerAddressTextField().setText(SettingsHandler.getGMGenOption(NetworkPlugin.LOG_NAME + ".ipAddress", "0.0.0.0"));
		initList();
	}
	...
	private void addListeners() {
		view.getServerRadioButton().addActionListener(new RadioActionListener());
		view.getClientRadioButton().addActionListener(new RadioActionListener());
		view.getConnectButton().addActionListener(new ConnectionButtonActionListener());
		view.getLogPane().addFocusListener(new TabFocusListener());
		view.getMessageButton().addActionListener(new MessageButtonActionListener());
	}
	...
	public void resetClient() {
		...
		getView().setConnectionText("Client Status", "Disconnected from Server");
		initList();
	}
	...
	public void log(String title, String message) {
		...
		try {
			logsDoc.insertAfterEnd(logsDoc.getCharacterElement(logsDoc.getLength()),
			 "<br>\n<b>" + dateFmt.format(Calendar.getInstance().getTime()) + " <font color='" + MiscUtilities.colorToHex(c) +
			 "'>" + title + "</b></font>: " + message);
			setPaneIcon();
		}
		catch (Exception e) {
			//damn
	...
	public void setPaneIcon() {
		...
		logPane.setIconAt(logPane.indexOfTab("Logs"), new javax.swing.ImageIcon(getClass().getResource("/pcgen/gui/resource/NewEnvelope.gif")));

		if (!(gmgenPane.getSelectedComponent() instanceof NetworkView)) {
			int index = gmgenPane.indexOfComponent(view);
			javax.swing.ImageIcon icon = new javax.swing.ImageIcon(getClass().getResource("/pcgen/gui/resource/NewEnvelope.gif"));
	...
	public void sendMessage() {
		try {
			if(server != null) {
				server.sendIM("GM", getSelectedUser(), view.getMessageTextField().getText());
			}
			else if(client != null) {
				if(getSelectedUser().equals("Broadcast")) {
					...
					client.sendIM(getSelectedUser(), view.getMessageTextField().getText());
				}
			}
		}
		finally {
