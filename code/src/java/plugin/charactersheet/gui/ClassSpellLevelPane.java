	...
	public void refresh() {
		if(serial < pc.getSerial()) {
			...
			if(!racial) {
				...
				perDay.setText(pcclass.getCastForLevel(level, Globals.getDefaultSpellBook(), pc) +
				 pcclass.getBonusCastForLevelString(level, Globals.getDefaultSpellBook(), pc));
	...
	private int addLine(CharacterSpell cs, String spellBook, Color color, int gridY, List<Component> colorList) {
		...
		effect.setText(
			new StringBuffer(EFFECT)
				.append(pc.parseSpellString(spell, spell.getDescription(pc), cs.getOwner())).toString()
		);
		gridBagConstraints = new java.awt.GridBagConstraints();
		gridBagConstraints.gridx = 1;
		gridBagConstraints.gridy = gridY;
