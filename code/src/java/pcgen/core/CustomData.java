...
public final class CustomData
{
	...
	public static void writeCustomItems()
	{
		...
		try
		{
			...
			for (Iterator e = EquipmentList.getEquipmentListIterator(); e
				.hasNext();)
			{
	...
	public static void writePurchaseModeConfiguration()
	{
		...
		final SortedMap pbStatCosts = SettingsHandler.getGame()
			.getPointBuyStatCostMap();
		...
		try
		{
			...
			if (pbStatCosts.size() > 0)
			{
				for (Iterator e = pbStatCosts.keySet().iterator(); e.hasNext();)
				{
					...
					final PointBuyCost pbc = ((PointBuyCost) pbStatCosts
						.get(statValue));
					bw.write("STAT:" + statValue.toString() + "\t\tCOST:"
						+ Integer.toString(pbc.getStatCost()));
	...
	private static void writeCustomBioSet()
	{
		...
		try
		{
			...
			for (Iterator it = Globals.getRaceMap().values().iterator(); it
				.hasNext();)
			{
	...
	{
		writeCustomPObjects(customClassFilePath(true), Globals.getClassList()
			.iterator());
	}

	private static void writeCustomDeities()
	{
		writeCustomPObjects(customDeityFilePath(true), Globals.getDeityList()
			.iterator());
	}

	private static void writeCustomDomains()
	{
		writeCustomPObjects(customDomainFilePath(true), Globals.getDomainList()
			.iterator());
	}
	...
	{
		writeCustomPObjects(customLanguageFilePath(true), Globals
			.getLanguageList().iterator());
	}

	private static void writeCustomPObjects(final String filename,
		final Iterator it)
	{
	...
	{
		writeCustomPObjects(customSkillFilePath(true), Globals.getSkillList()
			.iterator());
	}
	...
	private static void writeCustomSpells()
	{
		...
		try
		{
			...
			while (e.hasNext())
			{
				...
				{
					for (Iterator e2 = ((ArrayList) obj).iterator(); e2
						.hasNext();)
					{
	...
	{
		writeCustomPObjects(customTemplateFilePath(true), Globals
			.getTemplateList().iterator());
	}
}
