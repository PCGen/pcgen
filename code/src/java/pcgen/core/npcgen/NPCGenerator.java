...
public final class NPCGenerator
{
	...
	private WeightedCollection<SkillChoice> getSkillWeights(final PCClass aClass, final PlayerCharacter aPC)
	{
		...
		final PCClass aClass, final int level)
	{
		...
		Logging.debugPrint("NPCGenerator: Selecting " + skillPts //$NON-NLS-1$
			+ " skill points for " + aClass + "/" + level); //$NON-NLS-1$ //$NON-NLS-2$
		...
		while (skillPts > 0)
		{
			...
			Logging.debugPrint("NPCGenerator: Adding " + (int) ranks + "ranks"); //$NON-NLS-2$ 
			if (!Globals.checkRule(RuleConstants.SKILLMAX))
			{
				...
				if (pcRanks + ranks > maxRanks)
				{
					...
					for (SkillChoice skillChoice : skillList)
					{
						...
						if (chkSkill != null)
						{
							if (aPC.getRank(chkSkill).doubleValue() < aPC.getMaxRank(chkSkill, aClass).doubleValue())
							{
								ranksLeft = true;
								break;
							}
	...
	{
		final WeightedCollection<PCStat> stats =
				new WeightedCollection<>(theConfiguration.getStatWeights(aClass.getKeyName()));
	...
	private void generateStats(final PlayerCharacter aPC, final PCClass aClass, final RollMethod aRollMethod)
	{
		...
		{
			Logging.debugPrint(
				"NPCGenerator: Setting stat " + stat.getKeyName() //$NON-NLS-1$
				+ " to " + aPC.getStat(stat)); //$NON-NLS-1$
	...
	private WeightedCollection<Ability> getFeatWeights(final PCClass aClass)
	{
		...
		if (weightedCollection == null)
		{
			...
			// any other type
			for (Ability ability : Globals.getContext().getReferenceContext().getManufacturerId(AbilityCategory.FEAT)
				.getAllObjects())
			{
				int weight = 1;
	...
	private void selectFeats(final PlayerCharacter aPC, final WeightedCollection<Ability> aFeatList)
	{
		while ((int) aPC.getRemainingFeatPoolPoints() > 0)
		{
			...
			AbilityUtilities.driveChooseAndAdd(
				CNAbilityFactory.getCNAbility(AbilityCategory.FEAT, Nature.NORMAL, ability), aPC, true);
		}
	}
	...
	private void selectDeity(final PlayerCharacter aPC, final PCClass aClass)
	{
		...
		final WeightedCollection<Deity> deities =
				new WeightedCollection<>(theConfiguration.getDeityWeights(aClass.getKeyName()));
	...
	{
		final WeightedCollection<Domain> domains =
				theConfiguration.getDomainWeights(aPC.getDeity().getKeyName(), aClass.getKeyName());
	...
	private WeightedCollection<Spell> getKnownSpellWeights(PlayerCharacter pc, final PCClass aClass, final int aLevel)
	{
		WeightedCollection<Spell> WeightedCollection =
				theConfiguration.getKnownSpellWeights(pc, aClass.getKeyName(), aLevel);
		if (WeightedCollection == null)
		{
			WeightedCollection = new WeightedCollection<>();
			for (final Spell spell : pc.getSpellsIn(aClass.get(ObjectKey.CLASS_SPELLLIST), aLevel))
			{
	...
	private WeightedCollection<Spell> getPreparedSpellWeights(PlayerCharacter pc, final PCClass aClass,
		final int aLevel)
	{
		WeightedCollection<Spell> WeightedCollection =
				theConfiguration.getPreparedSpellWeights(aClass.getKeyName(), aLevel, pc);
		if (WeightedCollection == null)
		{
			WeightedCollection = new WeightedCollection<>();
			for (final Spell spell : pc.getSpellsIn(aClass.get(ObjectKey.CLASS_SPELLLIST), aLevel))
	...
	private void selectDomainSpell(final PlayerCharacter aPC, final PCClass aClass, final int aLevel)
	{
		...
		final WeightedCollection<Spell> domainSpells =
				new WeightedCollection<>(aPC.getSpellsIn(domain.get(ObjectKey.DOMAIN_SPELLLIST), aLevel));
		selectSpell(aPC, aClass, domain, "Prepared Spells", domainSpells, //$NON-NLS-1$
			aLevel);
	}

	private void selectSpell(final PlayerCharacter aPC, final PCClass aClass, final Domain aDomain,
		final String aBookName, final WeightedCollection<Spell> aSpellList, final int aLevel)
	{
		boolean added = false;
		while (!added)
		{
	 ...
	 */
	public void generate(final PlayerCharacter aPC, final AlignGeneratorOption align, final RaceGeneratorOption aRace,
		final GenderGeneratorOption aGender, final List<ClassGeneratorOption> classList,
		final List<LevelGeneratorOption> levels, final RollMethod aRollMethod)
	{
		...
		try
		{
			...
			for (int i = 0; i < MAX_RETRIES; i++)
			{
				...
				Logging.debugPrint("NPCGenerator: Selected " + r //$NON-NLS-1$
					+ " for race " + aRace); //$NON-NLS-1$
			...
			Logging.debugPrint("NPCGenerator: Selecting " + gender //$NON-NLS-1$
				+ " for gender " + aGender); //$NON-NLS-1$
			...
			for (int i = 0; i < classList.size(); i++)
			{
				...
				Logging.debugPrint("NPCGenerator: Selecting " + numLevels //$NON-NLS-1$
					+ " for level " + levels.get(i)); //$NON-NLS-1$
				PCClass aClass = null;
				...
				else
				{
					...
					for (;;)
					{
						...
						if (aClass.getSafe(ObjectKey.VISIBILITY) == Visibility.DEFAULT && aClass.qualifies(aPC, aClass))
						{
							Logging.debugPrint("NPCGenerator: Selecting " + aClass //$NON-NLS-1$
								+ " for class " + classList.get(i)); //$NON-NLS-1$
							break;
						}
				...
				for (int j = 0; j < numLevels; j++)
				{
					...
					if (pcClass.get(FactKey.valueOf("SpellType")) != null)
					{
						...
						if (aPC.getSpellSupport(pcClass).hasKnownList()
							|| aPC.getSpellSupport(pcClass).hasKnownSpells(aPC))
						{
							...
							for (int lvl = 0; lvl <= highestSpellLevel; ++lvl)
							{
								if (aPC.availableSpells(lvl, pcClass, Globals.getDefaultSpellBook(), true, true))
								{
									...
									//final int bonus = aPC.getSpellSupport(pcClass)
									//.getSpecialtyKnownForLevel(lvl, aPC);
									Logging.debugPrint("NPCGenerator: " + a //$NON-NLS-1$
										+ "known spells to select"); //$NON-NLS-1$

									final WeightedCollection<Spell> spellChoices =
											getKnownSpellWeights(aPC, pcClass, lvl);

									final int numToSelect = a - selectedSpells[lvl];
									for (int sp = 0; sp < numToSelect; sp++)
									{
										selectSpell(aPC, pcClass, null, Globals.getDefaultSpellBook(), spellChoices,
											lvl);
										selectedSpells[lvl]++;
									}
						...
						else
						{
							...
							for (int lvl = 0; lvl <= highestSpellLevel; ++lvl)
							{
								final int castTot = aPC.getSpellSupport(pcClass).getCastForLevel(lvl, "Prepared Spells",
									true, true, aPC);
								final int castNon = aPC.getSpellSupport(pcClass).getCastForLevel(lvl, "Prepared Spells",
									false, true, aPC);
								final int castSpec = castTot - castNon;
								Logging.debugPrint("NPCGenerator: " + castTot + "+" + castSpec //$NON-NLS-2$
									+ " prepared spells to select");
								...
								{
									final WeightedCollection<Spell> spellChoices =
											getPreparedSpellWeights(aPC, pcClass, lvl);
			...
			aPC.setPCAttribute(PCStringKey.HAIRSTYLE,
				globalHairStyleList.get(RandomUtil.getRandomInt(globalHairStyleList.size())));
			final List<String> speechList = SystemCollections.getUnmodifiableSpeechList();
			aPC.setPCAttribute(PCStringKey.SPEECHTENDENCY, speechList.get(RandomUtil.getRandomInt(speechList.size())));
			final List<String> globalPhobiaList = SystemCollections.getUnmodifiablePhobiaList();
			aPC.setPCAttribute(PCStringKey.PHOBIAS,
				globalPhobiaList.get(RandomUtil.getRandomInt(globalPhobiaList.size())));
			final List<String> globalInterestsList = SystemCollections.getUnmodifiableInterestsList();
			aPC.setPCAttribute(PCStringKey.INTERESTS,
				globalInterestsList.get(RandomUtil.getRandomInt(globalInterestsList.size())));
			final List<String> globalPhraseList = SystemCollections.getUnmodifiablePhraseList();
			aPC.setPCAttribute(PCStringKey.CATCHPHRASE,
				globalPhraseList.get(RandomUtil.getRandomInt(globalPhraseList.size())));
			final List<String> globalTraitList = SystemCollections.getUnmodifiableTraitList();
			// TODO: it is possible for trait1 == trait2
			aPC.setPCAttribute(PCStringKey.PERSONALITY1,
				globalTraitList.get(RandomUtil.getRandomInt(globalTraitList.size())));
			aPC.setPCAttribute(PCStringKey.PERSONALITY2,
				globalTraitList.get(RandomUtil.getRandomInt(globalTraitList.size())));

			final List<String> globalCityList = SystemCollections.getUnmodifiableCityList();
			aPC.setPCAttribute(PCStringKey.RESIDENCE,
				globalCityList.get(RandomUtil.getRandomInt(globalCityList.size())));
			final List<String> globalLocationList = SystemCollections.getUnmodifiableLocationList();
			aPC.setPCAttribute(PCStringKey.LOCATION,
				globalLocationList.get(RandomUtil.getRandomInt(globalLocationList.size())));
			final List<String> globalBirthplaceList = SystemCollections.getUnmodifiableBirthplaceList();
			aPC.setPCAttribute(PCStringKey.BIRTHPLACE,
				globalBirthplaceList.get(RandomUtil.getRandomInt(globalBirthplaceList.size())));
