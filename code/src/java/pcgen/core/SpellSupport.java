...
public class SpellSupport implements Cloneable
{
	...
	private HashMap<String, Integer> spellLevelMap =
			new HashMap<String, Integer>();
	private DoubleKeyMap<String, String, Info> spellInfoMap =
	 ...
	 * @param preList
	 */
	public void addSpellLevel(String tagType, String className,
		String spellName, String spellLevel, List<Prerequisite> preList)
	{
		preReqSpellLevelMap.put(tagType + "|" + className + "|" + spellName,
			preList);
	 ...
	 * @return Map
	 */
	public Map<String, Integer> getSpellMapPassesPrereqs(int levelMatch,
		PlayerCharacter pc)
	{
				...
				|| ((levelMatch >= 0) && (levelInt == levelMatch)))
			{
				if (preReqSpellLevelMap.containsKey(key))
				{
					if (key.startsWith(CLASSSPELLCASTER))
					{
						...
						spellType =
								spellType.substring(0, spellType.indexOf(PIPE));

						if (ALL.equals(spellType)
							|| pc.isSpellCaster(spellType, 1))
						{
							if (PrereqHandler.passesAll(preReqSpellLevelMap
								.get(key), pc, null))
							{
								for (PCClass pcClass : pc.getClassList())
								{
									if (pcClass.getSpellType()
										.equals(spellType)
										...
										tempSb.append(pcClass.getSpellKey(pc))
											.append(PIPE).append(
												key.substring(key
													.lastIndexOf(PIPE) + 1));
										tempMap
											.put(tempSb.toString(), levelInt);
									}
								}
							}
						}
					}
					else if (PrereqHandler.passesAll(preReqSpellLevelMap
						.get(key), pc, null))
					{
	 ...
	 * @return Map
	 */
	public Map<String, Integer> getSpellInfoMapPassesPrereqs(String key1,
		String key2, PlayerCharacter pc)
	{
		final Map<String, Integer> tempMap = new HashMap<String, Integer>();
		...
		if (spellInfoMap.containsKey(key1, key2))
		{
			...
			if (preReqSpellLevelMap.containsKey(key))
			{
				if (si.name.startsWith(SPELLCASTER))
				{
					...
					if (ALL.equals(spellType) || pc.isSpellCaster(spellType, 1))
					{
							...
							.get(key), pc, null))
						{
							for (PCClass pcClass : pc.getClassList())
							{
								if (pcClass.getSpellType().equals(spellType)
									|| ALL.equals(spellType))
								{
									tempMap.put(pcClass.getSpellKey(pc),
										si.level);
								}
							}
						}
				...
				else if (PrereqHandler.passesAll(preReqSpellLevelMap.get(key),
					pc, null))
				{
	...
	public String getPCCText()
	{
		...
		SortedMap<String, StringBuffer> spellOutputMap =
				new TreeMap<String, StringBuffer>();
		...
		for (String tagType : tagTypeSet)
		{
			...
			for (String spellName : spellNameSet)
			{
				...
				List<Prerequisite> preReqList =
						preReqSpellLevelMap.get(tagType + "|" + spellInfo.name
							+ "|" + spellName);
				...
				else
				{
					...
					txt.append(getSpellLevelPccText(tagType, spellName,
						spellInfo, preReqList));
				}
			}
		}
	 ...
	 */
	private String getSpellLevelPccText(String tagType, String spellName,
		Info spellInfo, List<Prerequisite> preReqList)
	{
		StringBuffer txt = new StringBuffer();
