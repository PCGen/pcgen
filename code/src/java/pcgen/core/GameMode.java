...
public final class GameMode implements Comparable<Object>, GameModeFacade
{
	...
	private static final String statDisplayTextAppend = "+";
	...
	{
		return spellRangeMap.get(aRange);
	}
	...
	public int getPurchaseScoreMin(final PlayerCharacter pc)
	{
		...
		if (pointBuyStatCosts != null)
		{
			return pointBuyStatCosts.keySet().stream()
			                 .filter(statValue -> pointBuyStatCosts.get(statValue).qualifies(pc, null))
			                 .findFirst()
			                 .orElse(-1);
		}
		return lastStat;
