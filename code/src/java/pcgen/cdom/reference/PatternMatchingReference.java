		...
		CDOMReference<T>
{
	...
	{
		List<T> list = all.getContainedObjects()
		                  .stream()
		                  .filter(obj -> obj.getKeyName().startsWith(pattern))
	...
	{
		int count =
				(int) all.getContainedObjects().stream().filter(obj -> obj.getKeyName().startsWith(pattern)).count();
		return count;
	}
