 ...
 * 
 * @param <T>
 *            The Type of object contained in this FactGroup
 * @param <F>
...
public class FactGroup<T extends CDOMObject, F> implements ObjectContainer<T>
{
	 ...
	 * @param fi
	 *            The FactInfo indicating the underlying characteristics of the
	...
	public Collection<T> getContainedObjects()
	{
		if (cache == null)
		{
			List<T> setupCache = allObjects.getContainedObjects().stream().filter((Predicate<T>) this::contains).collect(Collectors.toList());
			cache = setupCache;
		}
