...
final class PCGVer2Creator implements IOConstants
{
	...
	private void appendArmorProfLines(StringBuffer buffer)
	{
		...
		if ((thePC.getDeity() != null)
			&& thePC.getDeity().containsListFor(selectedArmorProfListKey))
		{
			buffer.append(TAG_ARMORPROF).append(':').append(TAG_DEITY).append(
				'=').append(thePC.getDeity().getKeyName()).append(':');

			for (String prof : thePC.getDeity().getListFor(
				selectedArmorProfListKey))
			{
		...
		for (PCClass pcClass : thePC.getClassList())
		{
			...
			buffer.append(TAG_ARMORPROF).append(':').append(TAG_CLASS).append(
				'=').append(pcClass.getKeyName()).append(':');
		...
		for (Ability ability : thePC.aggregateFeatList())
		{
			...
			buffer.append(TAG_ARMORPROF).append(':').append(TAG_FEAT).append(
				'=').append(ability.getKeyName()).append(':');
		...
		for (Skill skill : thePC.getSkillList())
		{
			...
			buffer.append(TAG_ARMORPROF).append(':').append(TAG_SKILL).append(
				'=').append(skill.getKeyName()).append(':');
		...
		for (CharacterDomain cd : thePC.getCharacterDomainList())
		{
			if ((cd.getDomain() == null)
				|| !cd.getDomain().containsListFor(selectedArmorProfListKey))
			{
				continue;
			}

			buffer.append(TAG_ARMORPROF).append(':').append(TAG_DOMAIN).append(
				'=').append(cd.getDomain().getKeyName()).append(':');

			for (String prof : cd.getDomain().getListFor(
				selectedArmorProfListKey))
			{
				buffer.append(prof).append(':');
			}
		...
		{
			if (!eq.isEquipped()
				|| !eq.containsListFor(selectedArmorProfListKey))
			{
				continue;
			}

			buffer.append(TAG_ARMORPROF).append(':').append(TAG_EQUIPMENT)
				.append('=').append(eq.getName()).append(':');
		...
		for (PCTemplate template : thePC.getTemplateList())
		{
			...
			buffer.append(TAG_ARMORPROF).append(':').append(TAG_TEMPLATE)
				.append('=').append(template.getKeyName()).append(':');
	...
	{
		final List<Ability> allFeats =
				new ArrayList<Ability>(thePC
		...
		for (Ability feat : allFeats)
		{
			if (feat.isMultiples())
			{
				for (int it2 = 0; it2 < feat.getAssociatedCount(); ++it2)
				{
					...
					if (isInLevelAbilityList(removeList, feat, feat
						.getAssociated(it2)))
					{
						continue;
					}
					...
					buffer
						.append(EntityEncoder.encode(feat.getAssociated(it2)));
					buffer.append('|');
					buffer.append(TAG_TYPE).append(':');
					...
					for (; it3 < maxit3; ++it3)
					{
						...
						buffer.append(EntityEncoder.encode(feat
							.getElementInList(saveListKey, it3)));
					}
		...
		for (Skill skill : thePC.getSkillList())
		{
				...
				|| (skill.getOutputIndex() != 0))
			{
				...
				for (String classRanks : skill.getRankList())
				{
					...
					buffer.append('|');
					buffer.append(TAG_COST).append(':');
					buffer.append(Integer.toString(skill.costForPCClass(
						pcClass, thePC)));
					buffer.append('|');
	...
	private void appendSpellLines(StringBuffer buffer)
	{
		...
		for (PCClass pcClass : thePC.getClassList())
		{
			for (CharacterSpell cSpell : pcClass.getSpellSupport()
				.getCharacterSpell(null, Constants.EMPTY_STRING, -1))
			{
