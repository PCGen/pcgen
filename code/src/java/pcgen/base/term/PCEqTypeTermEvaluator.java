/**
 * pcgen.base.term.PCEqTypeTermEvaluator.java
		...
		extends BasePCTermEvaluator implements TermEvaluator
{
	...
	// I'm not convinced that converting this to a float makes any sense.
	// It was in the old broken formula parser but that could produce
	// a String.  I've included it here, but if it doesn't produce a 
	// number it returns zero.
	public Float resolve(PlayerCharacter pc)
	{
		final Token token = new EqTypeToken();
		final String sTok = token.getToken(originalText, pc, null);

		if (Pattern.matches(fpRegex, sTok))
		{
			return Float.valueOf(sTok);
		}
