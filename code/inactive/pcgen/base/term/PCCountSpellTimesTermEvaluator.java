/**
 * pcgen.base.term.PCCountSpellTimesTermEvaluator.java
		...
		extends BasePCTermEvaluator implements TermEvaluator
{
	...
	public Float resolve(PlayerCharacter pc)
	{
		String bookName = (classNum == -1) ? Globals.getDefaultSpellBook() :
						  (bookNum > 0)    ? pc.getSpellBooks().get(bookNum) :
		...
		if (!"".equals(bookName))
		{
			...
			if (classNum == -1)
			{
				...
				for (PObject cl : pc.getClassList())
				{
					final List<CharacterSpell> bookList =
							cl.getSpellSupport()
									.getCharacterSpell(null, bookName, -1);
			...
			else 
			{
				...
				if (pcClass != null)
				{
					csList = pcClass.getSpellSupport().getCharacterSpell(
