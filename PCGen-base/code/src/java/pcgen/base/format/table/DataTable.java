...
/**
 * A DataTable represents tabular data (stored in columns and rows). Each column has a
 * name and a specific Format for the data.
 * 
 * This class makes best-effort based on an assumed good behavior of other classes. This
 * assumes that all columns are added before any rows. Also, if the format of a column is
 * altered after rows are added, then the behavior of DataTable is not defined.
 */
public class DataTable
{
	 ...
	 * 
	 * This may only be done BEFORE addRow is called. Once addRow is called, this method
	 * will return an IllegalStateException.
	 * 
	 * The provided TableColumn is value-semantic. It is captured and stored by this
	 * DataTable and changes (such as changing the TableColumn name) will alter the
	 * contents of this DataTable.
	 * 
	 * @param column
		...
		if (columns.isEmpty())
		{
			...
			if (formatManager instanceof ComparableManager)
			{
				...
				TreeMap<Object, Object[]> treeMap = new TreeMap<>(
					((ComparableManager) formatManager).getComparator());
				dataByRow = treeMap;
			}
			else
			{
	 ...
	 * 
	 * Note that the provided row must conform to the format of data as defined by the
	 * columns in this DataTable.
	 * 
	 * The provided List is reference-semantic. The contents of the List are stored, but
	 * the list itself is not stored. Thus any modification of the list after this method
	 * completes will not alter the contents of the DataTable.
	 * 
	 * @param rowData
	...
	/**
	 * Returns true if the given String is the name of a column in this DataTable.
	 * 
	 * @param columnName
	 *            The name to be checked to see if it is a column in this DataTable.
	 * @return true if the given String is the name of a column in this DataTable
	 */
	 ...
	 * 
	 * @param i
	 *            The column number for which the FormatManager should be returned
	 ...
	 * 
	 * @param columnName
	 *            The name of the column for which the FormatManager should be returned
	 * @return The FormatManager for the column with the given name
	 */
	public FormatManager<?> getFormat(String columnName)
	{
		return columns.stream()
			.filter(column -> column.getName().equals(columnName)).findFirst()
	 ...
	 */
	public Object lookup(LookupType lookupType, Object lookupValue,
		String resultingColumn)
	{
	...
	/**
	 * An Enumeration of the legal lookup types used to determine which row of a table to
	 * use.
	 */
	public enum LookupType
	{
		...
		EXACT {
			@Override
			public <V> Function<? super Map<Object, V>, V> getRowFor(
				Object lookupValue)
			{
		...
		LASTLTEQ {
			...
			public <V> Function<? super Map<Object, V>, V> getRowFor(
				Object lookupValue)
			{
				return map -> ((NavigableMap<Object, V>) map)
					.floorEntry(lookupValue).getValue();
			}
		 ...
		 */
		public abstract <V> Function<? super Map<Object, V>, V> getRowFor(
			Object lookupValue);
